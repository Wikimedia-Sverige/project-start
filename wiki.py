...
class Wiki:
    ...
    def add_project_page(
        self, phab_id, phab_name, parameters, goals, goal_fulfillments
    ):
        ...
        if page.exists() and not self._overwrite:
            logging.warning(
                "Project page '{}' already exists. It will not be created.".format(
                    page.title()
                )  # noqa: E501
            )
        else:
            ...
            for subpage in self._config["subpages"]:
                ...
                if "add_goals_parameters" in subpage:
                    ...
                    # just copied.
                    template_key = list(subpage["add_goals_parameters"].keys())[0]
                    template_value = self._make_year_title(
                        subpage["add_goals_parameters"][template_key]
                    )
                    subpage_parameters[template_key] = Template(
                        template_value, parameters=goals
                    )
                    subpage_parameters[
                        "måluppfyllnad"
                    ] = self._create_goal_fulfillment_text(
                        goals.keys(), goal_fulfillments
                    )  # noqa:E123
                self._add_subpage(
                    name,
                    subpage["title"],
    ...
    def _add_subpage(self, project, title, template_name, template_parameters=None):
    ...
    def _add_page_from_template(
        self, namespace, title, template_name, template_parameters
    ):
        ...
        if page.exists() and not self._overwrite:
            logging.warning(
                "Page '{}' already exists. It will not be created.".format(page.title())
            )
        else:
    ...
    def _add_category_page(self, title, categories):
        ...
        if page.exists() and not self._overwrite:
            logging.warning(
                "Category page '{}' already exists. It will not be created.".format(
                    page.title()
                )  # noqa: E501
            )
        else:
    ...
    def add_year_pages(self):
        ...
        for raw_title, template_name in simple_pages.items():
            ...
            self._add_page_from_template(None, title, template_name, [self._year])
    ...
    def _add_projects_year_page(self):
        ...
        for program in self._programs:
            content += "== {} {} ==\n".format(program["number"], program["name"])
            for strategy in program["strategies"]:
                content += "=== {} {} ===\n".format(
                    strategy["number"], strategy["short_description"]
                )
    ...
    def _make_project_data_string(self, project):
        ...
        project_template = Template(":Projekt:{}/Projektdata".format(name))
    ...
    def parse_programs(self):
        ...
        operational_plan_page = Page(
            self._site,
            self._make_year_title(self._config["year_pages"]["operational_plan"]),
        )
        # Get table string. This assumes that it is the first table on
        # the page.
        table_string = str(
            mwp.parse(operational_plan_page.text).filter_tags(matches=ftag("table"))[0]
        )
        # Remove ref tags and links.
        table_string = re.sub(
            r"(<ref.*?>.*?</ref>|\[\[.*?\||\]\])", "", table_string, flags=re.S
        )
        ...
        for row in rows[1:]:
            ...
            if len(cells) == 3:
                ...
                program_name, program_number = re.match(
                    r"(.*)\s+<!--\s*(.*)\s*-->", cells[0]
                ).groups()
            ...
            if len(cells) >= 2:
                ...
                strategy, strategy_number, strategy_short = re.match(
                    r"(.*)\s*<!--\s*(\d+)\s*(.*)\s-->", cells[-2]
                ).groups()
                ...
                for project in self._get_projects_for_strategy(strategy_number):
                    # Add projects for this strategy.
                    self._programs[-1]["strategies"][-1]["projects"].append(project)
    ...
    def _add_program_overview_year_page(self):
        ...
        for p, program in enumerate(self._programs):
            content_parameter += Template(
                templates["program"],
                True,
                {"program": program["name"], "färg": config["colours"][p]},
            ).multiline_string()
            content_parameter += "\n"
            for strategy in program["strategies"]:
                content_parameter += Template(
                    templates["strategy"], True, [strategy["description"]]
                ).multiline_string()
                content_parameter += "\n"
                for goal in strategy["goals"]:
                    content_parameter += Template(
                        templates["goal"], True, [goal]
                    ).multiline_string()
                    content_parameter += "\n"
                for project in strategy["projects"]:
                    content_parameter += Template(
                        templates["project"], True, [project]
                    ).multiline_string()
                    content_parameter += "\n"
        ...
        self._add_page_from_template(
            ...
            title,
            templates["page"],
            {"år": self._year, "tabellinnehåll": content_parameter},
        )
    ...
    def _add_volunteer_tasks_page(self):
        ...
        self._add_page_from_template(None, title, config["template"], parameters)
    ...
    def single_project_info(self, project_id, sv_name):
        ...
        pages = ["Mall:Projektid", "Mall:Projektnamn"]
